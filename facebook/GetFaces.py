...
def make_request(query):
    ...
    try:
        ...
        print ("Error on URL: {0}".format(url))
        return []
    response_text = response.read().decode('UTF-8')
    print(response_text)
    response_dict = json.loads(str(response_text))
...
def convert_rgb_to_bgr(open_cv_image):
    try:
        new_image = cv2.CreateImage((open_cv_image.width, open_cv_image.height), cv2.IPL_DEPTH_8U, open_cv_image.channels)
        cv2.CvtColor(open_cv_image, new_image, cv2.CV_RGB2BGR)
    except:
...
#         img = urllib2.urlopen(photo_url).read()
        ...
        print ("Error fetching image: {0}".format(photo_url))
        return None
...
#     pil_image = Image.open(img)
    try:
        print(photo_url)
        open_cv_image = cv2.imread(photo_url) #fromarray(np.array(pil_image))[:, :]
    except TypeError:
        print ("unsupported image type")
        return None
    open_cv_image = convert_rgb_to_bgr(image)
    return image
...
def normalize_face_size(face):
    ...
    resized_face = cv2.fromarray(resized_face)
    return resized_face
...
def save_face(name, service_id, face, picture_name):
    ...
    print ("Saving: {0}".format(full_path))
